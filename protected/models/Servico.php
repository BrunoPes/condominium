...
class Servico extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('nomeServico, preco, usuarioId', 'required'),
			array('usuarioId', 'numerical', 'integerOnly'=>true),
			array('preco', 'numerical'),
			array('nomeServico', 'length', 'max'=>30),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, nomeServico, preco, usuarioId', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('nomeServico',$this->nomeServico,true);
